...
export default function WalletPage() {
  ...
  async function loadHistory() {
    await run(async () => {
      const data = await gatewayRequest<WalletTransaction[]>("wallet", `wallet/${session.userId}/transactions`, {
        headers: { Authorization: auth },
      });
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
        <div className="rounded-3xl border border-white/60 bg-white/85 p-6 shadow-lg shadow-orange-100/60 dark:border-slate-700 dark:bg-slate-900/80">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button onClick={loadWallet} disabled={loading} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60 dark:bg-slate-100 dark:text-slate-900">
              Muat Saldo
            </button>
            <button onClick={loadHistory} disabled={loading} className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold disabled:opacity-60 dark:border-slate-700">
              Muat Riwayat
            </button>
          </div>
        </div>
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          {transactions.length === 0 ? (
            <p className="mt-3 rounded-lg border border-dashed border-slate-300 p-4 text-sm text-slate-500 dark:border-slate-700">
              Belum ada transaksi. Klik &quot;Muat Riwayat&quot; untuk mengambil data terbaru.
            </p>
          ) : (
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {transactions.map((tx) => (
                <article key={tx.transactionId} className="rounded-xl border border-slate-200 p-4 dark:border-slate-800">
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-300">{tx.status}</p>
